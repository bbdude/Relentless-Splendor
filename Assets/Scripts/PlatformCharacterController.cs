...
public class PlatformCharacterController : MonoBehaviour
{
	...
	/// <summary>
	/// mask with all layers that should act as one-way platforms. Note that one-way platforms should always be EdgeColliders. This is because it does not support being
	...
	/// <summary>
	/// the max slope angle that the CC can climb
	/// </summary>
	...
	void Awake()
	{
		...
		// we want to set our CC to ignore all collision layers except what is in our triggerMask
		for( var i = 0; i < 32; i++ )
		{
	...
	[System.Diagnostics.Conditional( "DEBUG_CC_RAYS" )]
	...
	void moveHorizontally( ref Vector3 deltaMovement )
	{
      ...
      bool hitit = false;
			...
			if( i == 0 && collisionState.wasGroundedLastFrame )
				hitit = Physics.Raycast( ray, rayDirection, out _raycastHit, rayDistance, platformMask );
			else
				hitit = Physics.Raycast( ray, rayDirection, out _raycastHit, rayDistance, platformMask & ~oneWayPlatformMask );
	...
	bool handleHorizontalSlope( ref Vector3 deltaMovement, float angle )
	{
        ...
        bool hitit = false;
				...
				if( collisionState.wasGroundedLastFrame )
					hitit = Physics.Raycast( ray, deltaMovement.normalized, out raycastHit, deltaMovement.magnitude, platformMask );
				else
					hitit = Physics.Raycast( ray, deltaMovement.normalized, out raycastHit, deltaMovement.magnitude, platformMask & ~oneWayPlatformMask );
	...
	void moveVertically( ref Vector3 deltaMovement )
	{
		...
		for( var i = 0; i < totalVerticalRays; i++ )
		{
			...
			DrawRay( ray, rayDirection * rayDistance, Color.red );
			bool hitit = Physics.Raycast( ray, rayDirection, out _raycastHit, rayDistance, mask );
			if( hitit )
			{
				// set our new deltaMovement and recalculate the rayDistance taking it into account
	...
	private void handleVerticalSlope( ref Vector3 deltaMovement )
	{
		...
		bool hitit = Physics.Raycast( slopeRay, rayDirection, out _raycastHit, slopeCheckRayDistance, platformMask );
		if( hitit )
		{
